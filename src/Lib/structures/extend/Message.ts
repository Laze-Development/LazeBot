...
const searchQuery = (query: string, str: string) => new RegExp(`.*${ query.split(" ").join(".*") }.*`, "gi").test(str);

export class ExtendedMessage extends Structures.get("Message") {
    public sem(content: string, options: {
    ...
    public async find(type: "member" | "user", query: string): Promise<User | GuildMember | undefined | null> {
        if (type === "member") {
            ...
            return (this.guild!.members.cache.get(query) || this.guild!.members.cache.find((x) => searchQuery(query, x.user.username)) || null);
        } else {
            if (this.mentions.users.first()) return this.mentions.users.first();
            return (this.client.users.cache.get(query) || this.client.users.cache.find((u) => searchQuery(query, u.username)));
        }
    }
}
